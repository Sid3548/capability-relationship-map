"""Full comprehensive pipeline (A+B+C+D) for Qwen3.5-9B, int8, local dir.
Identical logic to scripts/run_full.py; only model loading swapped for the
int8/bnb/device_map=auto/local-dir/offline path used in scripts/smoke_qwen.py.
...
Usage: python -m scripts.run_full_qwen [outdir] [max_fraction]
"""
...
def main():
    outdir = REPO_ROOT / (sys.argv[1] if len(sys.argv) > 1 else "results/comprehensive_qwen35_9b")
