"""LLaMA-3.1-8B (int8) smoke slice: capture -> score -> 3-control removal sweep.
Mirrors the Qwen smoke interface (src/*). JSON/JSONL only (no pyarrow/mpl in torch proc).
Usage: python -m scripts.smoke_llama
"""
...
OUT = REPO_ROOT / "results" / "llama31_8b_smoke"
SEED = 1234
STEP_FRAC = 0.001      # 0.1% ~ 459 neurons/step
...
def main():
    ...
    json.dump({
        "model": "Meta-Llama-3.1-8B", "precision": "int8(bnb)", "seed": SEED,
